...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        {
            var dbSets = _dbSetProperties.Select(pi => pi.Value.GetValue(this))
                .ToArray();
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                {
                    throw new InvalidOperationException($"{invalidEntities.Length} Invalid" +
                        $"Entities Found in {dbSet.GetType().Name}");
                }
            }
        ...
        private void MapAllRelations()
        {
            foreach (var dbSetProperty in _dbSetProperties)
            {
                ...
                mapRelationsGeneric.Invoke(this, new
                    object[]
                {
                    dbSet
                });
            ...
            where TEntity : class, new()
        {
            ...
            foreach (var collection in collections)
            {
                ...
                var mapCollctionMethod = typeof(DbContext)
                    .GetMethod("MapCollection",
                    BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(entityType,collectionType);
                mapCollctionMethod.Invoke(this, new
                    object[]{
                        dbSet,collection
                });
            }
            ...
            where TCollection: class, new()
        {
            ...
            if (isManyToMany)
            {
                primaryKey = collectionType.GetProperties()
                    ...
                    .GetProperty(pi.GetCustomAttribute<ForeignKeyAttribute>()
                    .Name)
                    .PropertyType == entityType);
            }

            var navigationDbSet = (DbSet<TCollection>)
                _dbSetProperties[collectionType].GetValue(this);
            foreach (var entity in dbSet)
            {
                ...
                ReflectionHelper.ReplaceBackingField(entity,
                    collectionProperty.Name,navigationEntities);
            }
        }
    
        private void MapNavigationProperties<TEntity>(
            DbSet<TEntity> dbSet)
            where TEntity : class, new()
        {
            var entityType = typeof(TEntity);
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue =
                        ...
                        navigationPrimaryKey.GetValue(
                            currentNavigationProperty).Equals
                            (foreignKeyValue));
                    
                    navigationProperty.SetValue(entity, 
        ...
        private Dictionary<Type, PropertyInfo> DiscoverDbSets()
        {
            var dbSets = GetType().GetProperties()
                .Where(pi=>pi.PropertyType.GetGenericTypeDefinition()
                == typeof(DbSet<>))
